...
export class PeCouponsGridComponent implements OnInit, OnDestroy {
  ...
  public folderAction(event: FolderOutputEvent, action: PeFoldersActionsEnum): void {
    switch (action) {
      case PeFoldersActionsEnum.Update:
        this.store.dispatch(new PeFoldersActions.Update(event.data, event.data._id));
        break;
      case PeFoldersActionsEnum.Delete:
  ...
  private openEditor(gridItem?: PeGridItem, actionData?: PeFolderEditorActionDataInterface): void {
    ...
    const overlayRef = this.peOverlayWidgetService.open(config);

    saveSubject$.pipe(
      filter(coupon => !!coupon),
      tap((coupon: PeCouponInterface) => {
        const couponItem = this.peCouponsGridService.couponsToGridItemMapper([coupon], this.canvas)[0];
        const action = gridItem
          ? new PeGridItemsActions.EditItem(couponItem)
          : new PeGridItemsActions.AddItem(couponItem);
