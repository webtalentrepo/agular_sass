...
export class EditPictureComponent {
  ...
  private updatePictures(): void {
    this.changePictures.emit({ image: this.peMediaService.getMediaUrl(this.picture, 'builder'), type: this.type });
  }
  ...
  private addPictures(files: File[]): void {
    ...
    this.peMediaService.postMediaBlob(files[0], files[0].type as PeMediaFileTypeEnum, this.businessId).pipe(
      finalize(() => {
