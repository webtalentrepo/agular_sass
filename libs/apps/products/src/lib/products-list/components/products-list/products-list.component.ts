...
export class ProductsListComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  changeStoreItem(itemData: PeGridItem, newItem: ProductModel | FolderItem, action: ProductsFolderAction): void {
    const isCurrentFolder = (this.selectedFolder?._id ?? null) === (newItem.parentFolderId ?? null);
    const addStoreItem = () => {
  ...
  ngOnInit(): void {
    ...
    this.initProducts();
    this.productsRulesService.initRuleListener();
    ...
    request.pipe(
      switchMap((products) => {
        ...
        products.collection.forEach((res, i) => {
          ...
          if (moment().diff(moment(res.createdAt)) < 5000) {
