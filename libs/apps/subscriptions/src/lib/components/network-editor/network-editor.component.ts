...
import { PE_SUBSCRIPTIONS_FIRST_PLAN } from '../../constants';
...
@Component({
  selector: 'pe-subscriptions-network-editor',
  templateUrl: './network-editor.component.html',
  styleUrls: ['../form-preloader.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [PeDestroyService],
})
export class PeSubscriptionsNetworkEditorComponent implements OnInit {
  ...
  private readonly createBtn = this.translateService.translate('subscriptions-app.actions.create');
  private readonly deleteBtn = this.translateService.translate('subscriptions-app.actions.delete');
  private readonly loadingBtn = this.translateService.translate('subscriptions-app.actions.loading');
  private readonly openBtn = this.translateService.translate('subscriptions-app.actions.open');
  ...
  private get errorMessage(): string {
    ...
    if (errors?.required) {
      return 'subscriptions-app.network_editor.network_name.error.required';
    } else if (errors?.incorrectDomainName) {
      return 'subscriptions-app.network_editor.network_name.error.incorrect';
    } else if (errors?.minLengthDomainName) {
      return 'subscriptions-app.network_editor.network_name.error.min_name_length';
    } else if (!this.isUnique) {
      return 'subscriptions-app.network_editor.network_name.error.not_unique';
    } else {
      return 'subscriptions-app.network_editor.network_name.error.incorrect';
    }
  }
  ...
  private closeEditor = () => {
    if (this.networkForm.dirty && !this.loading) {
      ...
      const headingTitle = isEditing
        ? 'subscriptions-app.confirm_dialog.cancel.network_editor.editing.title'
        : 'subscriptions-app.confirm_dialog.cancel.network_editor.creating.title';
      const headingSubtitle = isEditing
        ? 'subscriptions-app.confirm_dialog.cancel.network_editor.editing.subtitle'
        : 'subscriptions-app.confirm_dialog.cancel.network_editor.creating.subtitle';
  ...
  public removeNetwork(networkId: string): void {
      ...
      .pipe(
        ...
        tap(() => {
          ...
          const notify = this.translateService
            .translate('subscriptions-app.notify.network_deleted')
    ...
    const headingTitle = 'subscriptions-app.confirm_dialog.delete.network.title';
    const headingSubtitle = 'subscriptions-app.confirm_dialog.delete.network.subtitle';
    const config: Headings = {
      title: this.translateService.translate(headingTitle),
      subtitle: this.translateService.translate(headingSubtitle),
      ...
      of(_id.value)
        .pipe(
          switchMap(networkId => {
            ...
            return networkId
              ? this.peSubscriptionsApiService.updateNetwork(networkId, network)
              : this.peSubscriptionsApiService.createNetwork(network);
          }),
          switchMap(network => {
            ...
            const createFirstProgram$ = isNewNetwork
              ? this.peSubscriptionsApiService.createPlan(PE_SUBSCRIPTIONS_FIRST_PLAN)
              : of(null);
            const setNetworkAsDefault$ = !network.isDefault
              ? this.peSubscriptionsApiService.setNetworkAsDefault(network._id)
              : of({ _id: network._id });
          ...
          switchMap(([{ _id }, plan]) => this.peSubscriptionsAccessApiService.getAccessConfig(_id)),
          switchMap((accessConfig: PeSubscriptionsNetworkAccessInterface) => {
            const { subscriptionNetwork, _id, createdAt, updatedAt } = accessConfig;
            const condition = createdAt === updatedAt
              ? 'subscriptions-app.notify.network_created'
              : 'subscriptions-app.notify.network_updated';
            const notify = this.translateService
              .translate(condition)
            ...
            return this.peSubscriptionsAccessApiService
              .updateAccessConfig(subscriptionNetwork, _id, { internalDomain: name.value });
          }),
          tap(() => {
  ...
  public validateName(event): void {
    ...
    !invalid && enteredName.length > 2 && this.peSubscriptionsApiService
