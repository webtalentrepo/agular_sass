...
import { ActionsListService, MAX_MAIN } from './actions-list.service';
...
export class ActionsListComponent implements OnInit {
    ...
    this.apiService.checkSantanderStatus(this.orderId)
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          this.actionsListService.showError(error.error);
        },
      });
  }

  onSendSigningLinkAction(): void {
    this.apiService.postAction(this.settingsService.businessUuid, this.orderId, ActionTypeEnum.SendSigningLink, {}).pipe(
      switchMap((data: DetailInterface) => this.detailService.getData(this.orderId, true).pipe(
        tap((detail: DetailInterface) => {
          this.order = { ...detail, actions: data.actions };
        }),
      )),

      catchError((error) => {
        this.actionsListService.showError(error);
  ...
  private updateDetails(): void {
    this.detailService.getData(this.orderId, true).pipe(
      take(1)
    ).subscribe();
  }
  ...
  private onClickOrderShippingAction(order: ActionInterface): void {
    if (order.action === ActionTypeEnum.DownloadShippingSlip) {
      this.onClickOrderAction(ActionTypeEnum.DownloadShippingSlip);
    }
