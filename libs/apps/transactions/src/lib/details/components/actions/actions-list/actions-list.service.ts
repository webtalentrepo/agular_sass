...
import { ActionInterface, ActionMapInterface, ActionTypeEnum, DetailInterface } from '../../../../shared';
...
export class ActionsListService {
  ...
  private shippingGoodsClick(action: ActionInterface, order:DetailInterface ): void {
    if (order.shipping.example_label) {
      ...
      this.detailService.actionOrder(
        this.orderId, {}, ActionTypeEnum.ShippingGoods, 'payment_shipping_goods', true, order.payment_option.type
      ).subscribe(
        () => {
          this.onClickOrderAction(action.action);
  ...
  private onClickOrderAction(action: ActionTypeEnum): void {
    if (action) {
