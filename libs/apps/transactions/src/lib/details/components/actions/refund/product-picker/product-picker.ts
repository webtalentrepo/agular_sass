...
export class RefundProductPickerComponent implements OnInit {
  ...
  onChanges(e: Event, index: number): void {
    const { value, max } = e.target as HTMLInputElement;
    (this.refundForm.get('refundItems') as FormArray).controls[index].get('quantity').setValue(
      Math.max(0, Math.min(Number(max), parseInt(value, 10))),
      {
        emitEvent: false,
      }
    )
