...
describe('PeChatFormComponent', () => {
  ...
  it('the text to be typed is lost when any new message received', () => {
    const eventInit: KeyboardEventInit = {
      key: 'S',
    };
    spyOn(component.draftUpdates, 'emit');

    component.checkDraftUpdates();
    const event = new KeyboardEvent('keyup', eventInit);
    component.onKeyUp(event);

    fixture.detectChanges();

    expect(component.draftUpdates.emit).toHaveBeenCalled();
  });
