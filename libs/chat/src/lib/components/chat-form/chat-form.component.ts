...
export class PeChatFormComponent implements OnInit, AfterViewInit {
  ...
  @Output() draftUpdates = new EventEmitter<{ draftMessage?: string; }>();
  ...
  checkDraftUpdates(): void {
    this.draftUpdates.emit({ draftMessage: this.textarea.value });
  }
  ...
  onKeyUp($event): void {
    ...
    if (message && $event.key !== KeyEventEnum.ENTER) {
      this.inputChange$.next();
    }
    if (message) {
      this.checkDraftUpdates();
    }
    if (
      ($event.key === KeyEventEnum.ENTER && $event.shiftKey)
      || ($event.key === KeyEventEnum.ENTER && $event.metaKey)
