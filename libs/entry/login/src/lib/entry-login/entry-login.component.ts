...
import { BehaviorSubject, EMPTY, interval, Observable, of, zip } from 'rxjs';
import {
  catchError,
  delay,
  filter,
    ...
    this.authService
      ...
      .pipe(
        ...
        catchError((errors: any) => {
          ...
          if (errors.raw && errors.raw.reason && CAPTCHA_REASONS.indexOf(errors.raw.reason) >= 0) {
            this.form.addControl('recaptchaToken', new FormControl('', [Validators.required]));
          }

          if (errors.raw && errors.raw.reason && BLOCK_REASONS.indexOf(errors.raw.reason) >= 0) {
            this.errorMessage = this.translateService.translate(`forms.login.error_reasons.${errors.raw.reason}`);
          } else if (errors.raw && errors.raw.reason && CAPTCHA_REASONS.indexOf(errors.raw.reason) >= 0) {
            this.errorMessage = errors.message || 'Unknown error';
          } else if (errors.raw && errors.raw.message && EMAIL_REASONS.indexOf(errors.raw.message) >= 0) {
