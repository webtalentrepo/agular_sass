...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const mainStreamArr$ = [
      ...
      this.store.select(state => state.peUserState.peBusinessState).pipe(
        tap(() => {
    ...
    merge(...mainStreamArr$).pipe(takeUntil(this.destroyed$)).subscribe();

    this.localeService.currentLocale$.pipe(
      tap((locale) => {
        this.helpHref = this.contactHref = locale.code === 'de'
          ? 'https://support.payever.org/hc/de' : 'https://support.payever.org/hc/en-us';
      }),
  ...
  addBusiness() {
    const config: PeOverlayConfig = {
      ...
      headerConfig: {
        ...
        backBtnTitle: 'Cancel',
        theme: this.theme,
        backBtnCallback: () => { this.peOverayWidgetService.close() },
        cancelBtnTitle: '',
        cancelBtnCallback: () => { },
        doneBtnTitle: 'Done',
        doneBtnCallback: () => { },
    ...
    this.apiService.getBusinessRegistrationData().pipe(
      take(1),
      tap((data) => {
  ...
  personalInformation() {
    this.router.navigate([`personal/${this.authService.getUserData().uuid}`]).then(() => {
      this.peMessageOverlayService.closeMessages();
    })
  }
