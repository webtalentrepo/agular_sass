...
export class PeMessageChatRoomSettingsComponent implements OnInit {
  ...
  peMessageChannelPermissions = PeMessageChannelPermissionsEnum;
  ...
  ngOnInit(): void {
    const initActiveConversationSettings$ = this.peMessageChatRoomListService.activeChat$.pipe(
      take(1),
      tap(activeChat => {
        this.admins = this.getMembersByCategory(true, activeChat.membersInfo);
        this.subscribers = this.getMembersByCategory(false, activeChat.membersInfo);
  ...
  private getMembersByCategory(searchForAdmins: boolean, membersInfo: any[]): PeMessageChannelMemberByCategory[] {
    ...
    return (
      membersInfo
        .map(member => (roles.includes(member?.role) ? this.peMessageChatRoomListService.getMember(member) : null))
  ...
  private deleteChannel(): void {
    ...
    this.confirmScreenService
      ...
      .pipe(
        take(1),
        filter(Boolean),
        switchMap(() => this.peMessageChatRoomListService.deleteChat(this.activeChat?._id)),
        tap(() => {
          this.closeForm();
