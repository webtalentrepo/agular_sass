...
import { BehaviorSubject, merge, Observable, of, Subject, EMPTY, Subscription } from 'rxjs';
...
export class PeMessageChatRoomComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('deleteMessageEveryoneTemplate', { static: true }) deleteMessageEveryoneRef;
  ...
  public draftUpdates({ draftMessage }): void {
    this.peMessageConversationService.draftUpdateChange({ draftMessage });
  }
  ...
  draft({ chatId, draftMessage }: PeMessageChatDraft): void {
    const activeChat = Object.assign({}, chatId
      ? this.peMessageConversationService.conversationList$.value
        .find(conversation => conversation.id === chatId).data
      : this.peMessageChatRoomListService.activeChat);
  ...
  private getCopyText(message: PeChatMessage): string {
    ...
    textParts.push(
      `${message.name} [${moment(message.sentAt).format('DD.MM.YY')} ${moment(message.sentAt).format('HH:mm')}]`,
    );

    if (message.replyData) {
      textParts.push(`[In reply to ${message.replyData.name}]`);
    }
  ...
  openChatFormContextMenu(event: MouseEvent): void {
    ...
    dialogRef.afterClosed
      .pipe(
        take(1),
        tap(res => {
          if (res === PeMessageChatRoomContextMenu.Paste) {
  ...
  showConfirmationDialog(chat: PeMessageChat, messages: PeChatMessage[]) {
    ...
    const headings: Headings = {
      ...
      customBottomTemplate: this.deleteMessageEveryoneRef,
    };
