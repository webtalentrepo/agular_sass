...
export class PeMessageConversationService {
  ...
  draftUpdateChange({ draftMessage }) {
    if (draftMessage) {
      const activeChat = cloneDeep(this.peMessageChatRoomListService.activeChat);
      if (!activeChat.draft) {
        activeChat.draft = {};
      }
      activeChat.draft.content = draftMessage;
      activeChat.draft.sender = this.peMessageService.getUserData().uuid;
      this.peMessageChatRoomListService.activeChat = activeChat;

      const activeChatMessage = this.conversationList$.value.find(conversation => conversation.id === activeChat._id)
        .data;

      if (!activeChatMessage.draft) {
        activeChatMessage.draft = {};
      }
      activeChatMessage.draft.content = draftMessage;
      activeChatMessage.draft.sender = this.peMessageService.getUserData().uuid;
    }
  }
